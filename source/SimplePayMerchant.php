...
class SimplePayMerchant {
    ...
    public function __contrsuct($config){

        $required_keys = array(
            'outlet_id', 'secret_key', 'secret_key_result', 'result_url'
        );


        foreach ($required_keys as $required_key) {
            if (!isset($config[$required_key])) {
                throw new \Exception('Please set up ' . $required_key . ' param for simple pay component');
            }
        }

        if (isset($config['strong_ssl'])) {
            $this->strong_ssl = (boolean)$config['strong_ssl'];
        }
     ...
     */
    function process_result_request($process_success, $process_fail) {
        ...
        if (empty($REQUEST_PARAMS['sp_sig'])) {
            throw new \Exception("Некорректные входящие параметры");
        }
        ...
        if ($REQUEST_PARAMS['sp_sig'] != $right_sig) {
            throw new \Exception("Некорректная подпись запроса");
        }
        ...
        if ($REQUEST_PARAMS['sp_result'] == 1) {
            ...
            $process_success($order_id, $REQUEST_PARAMS);
            $answer_description = "Оплата принята";
        } else {
            // заказ с номером $order_id не будет оплачен.
            $process_fail($order_id, $REQUEST_PARAMS);
